...
int main(int argc, char *argv[]){
	...
	while(1){	
		...
		if(pid == 0){
			...
			while(1){
				/* Get File Name */
				int len = recvfrom(sockfd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&client, (socklen_t *)&client_len);
				...
				while(1){
					...
					send_size += data_size;
					if(send_size == (int)pow(2, 10+tmp) && cwnd >= 1024 ){
						done = true, send_size = 0;
					}else if(send_size == cwnd && cwnd < 1024){
						done = true, send_size = 0;
					}
					if(data_size == 0) done = true, send_size = 0, fin = true;
					...
					send_pkt.ack = 1;
					send_pkt.ack_num = recv_pkt.seq_num + 1;
					pre_seq = send_pkt.seq_num = file_num;
					...
					pre_seq += data_size;
					//printf("%d %d\n", pre_seq, data_size);
					if(file_num == 4096 && !lossed){
						lossed = true;
						if(cwnd < 1024) printf("\tSend a packet at : %d bytes ***loss\n", read_byte);
						else printf("\tSend a packet at : %d bytes ***loss\n", read_byte);
					}else if(data_size != 0){
						sendto(sockfd, pkt_data, data_size + 24, 0, (struct sockaddr *)&client, client_len);
					...
					while(done){
						...
						/* Check recv_pkt info */
						if(recv_pkt.ack && recv_pkt.ack_num <= pre_seq){
							printf("\tReceive a packet (seq_num = %d, ack_num = %d)\n", recv_pkt.seq_num, recv_pkt.ack_num); 
							
							if(preACK == recv_pkt.ack_num) duplicate++;
							else duplicate = 0;
							
							if(duplicate == 3){
								printf("Receive three duplicated ACKs.\n");
								...
								pre_seq = recv_pkt.ack_num;
								file_num = recv_pkt.ack_num;
								break;
							}
							
							preACK = recv_pkt.ack_num;
